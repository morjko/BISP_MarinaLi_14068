...
import { FaCat, FaDog, FaEllipsisH, FaFrog, FaKiwiBird, FaMapMarkedAlt } from "react-icons/fa";
    ...
    <div>
      ...
      {advert && !error && (
        <div className="max-w-4xl mx-auto" style={{ display: "flex" }}>
          <div className="max-w-2/3 w-1/2 h-[550px] text-green-600 flex flex-col gap-4 place-content-center mr-10">
            <div>
              <p className="text-4xl font-semibold">{advert.name}</p>
            </div>
            <div className="flex items-center gap-2">
              <FaMapMarkedAlt className="text-rose-400" />
              <p>{advert.location} - <span className="font-bold">{advert.price}</span> UZS / hour</p>
            </div>
            <div>
              <p className="font-bold">About <span className="text-sm">{advert.name}</span></p>
