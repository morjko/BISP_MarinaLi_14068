...
export const google = async (req, res, next) => {
    try {
        ...
        if (user) {
            ...
            const hashPassword = bcryptjs.hashSync(autoGeneratedPassword, 6);
            const newUser = new User({username: req.body.name.split(" ").join("").toLowerCase()+Math.random().toString(36).slice(-8), email: req.body.email, password: hashPassword, ava: req.body.photo});
            await newUser.save();
